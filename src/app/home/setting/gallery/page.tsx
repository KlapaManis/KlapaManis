...
export default function GallerySettingPage(){
  ...
  const [form,setForm]=useState<any>({imageUrl:'',title:'',deskripsi:''})
  ...
  async function submit(e:React.FormEvent){
    ...
    setForm({imageUrl:'',title:'',deskripsi:''}); setEditId(null); load()
  }

  async function del(id:number){ if(!confirm('Hapus?'))return; await fetch(`/api/gallery/${id}`,{method:'DELETE'}); load() }
  function edit(r:GalleryRow){ setEditId(r.id); setForm({imageUrl:r.imageUrl,title:r.title||'',deskripsi:r.deskripsi||''}) }
    ...
    <div className="max-w-5xl mx-auto p-4 space-y-4">
      ...
      <form onSubmit={submit} className="bg-white border rounded-xl p-4 grid sm:grid-cols-2 gap-3">
        ...
        <div className="sm:col-span-2 flex gap-2">
          ...
          {editId && <button type="button" onClick={()=>{setEditId(null);setForm({imageUrl:'',title:'',deskripsi:''})}} className="border rounded-lg px-4 py-2 text-sm">Batal</button>}
        </div>
      </form>
